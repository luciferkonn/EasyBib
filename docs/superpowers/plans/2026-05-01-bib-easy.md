# EasyBib Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a Claude Code plugin (`easybib`) that validates, resolves, fills, and organizes bibtex citations against DBLP via four slash commands (`/check`, `/suggest`, `/create`, `/organize`) and one auto-triggering skill.
...
**Spec:** `docs/superpowers/specs/2026-05-01-easybib-design.md`
...
- `.gitignore` — excludes `.easybib-cache/`, `__pycache__`, `.pytest_cache`.
...
**Python scripts (Tasks 2–9, in `skills/easybib/scripts/`):**
...
- `changelog.py` — append JSONL records to `.easybib.log`.
- `config.py` — load `.easybib.toml` with defaults.

**Skill files (Tasks 10–11, in `skills/easybib/`):**
...
- **Agent skill** = a `SKILL.md` with YAML frontmatter (`name`, `description`) at the top. Additional `.md` files and a `scripts/` folder bundle context that loads only when Claude asks for it (progressive disclosure). See `docs/superpowers/specs/2026-05-01-easybib-design.md` §2.
...
- Create: `skills/easybib/scripts/requirements.txt`
- Create: `skills/easybib/scripts/requirements-dev.txt`
...
- [ ] **Step 6: Create `skills/easybib/scripts/requirements.txt`**
...
- [ ] **Step 7: Create `skills/easybib/scripts/requirements-dev.txt`**
...
```bash
git add plugin.json .gitignore LICENSE Makefile pyproject.toml README.md skills/easybib/scripts/requirements.txt skills/easybib/scripts/requirements-dev.txt
...
**Files:**
- Create: `skills/easybib/scripts/bib_parse.py`
...
- [ ] **Step 5: Implement `skills/easybib/scripts/bib_parse.py`**
...
Run: `ruff check skills/easybib/scripts/bib_parse.py`
Expected: exits 0.

- [ ] **Step 8: Commit**

```bash
git add skills/easybib/scripts/bib_parse.py tests/test_bib_parse.py tests/fixtures/bib/
...
**Files:**
- Create: `skills/easybib/scripts/tex_resolve.py`
...
- [ ] **Step 4: Implement `skills/easybib/scripts/tex_resolve.py`**
...
Run: `ruff check skills/easybib/scripts/tex_resolve.py`
Expected: 0.

- [ ] **Step 7: Commit**

```bash
git add skills/easybib/scripts/tex_resolve.py tests/test_tex_resolve.py tests/fixtures/tex/
...
**Files:**
- Create: `skills/easybib/scripts/tex_scan.py`
...
- [ ] **Step 4: Implement `skills/easybib/scripts/tex_scan.py`**
...
Run: `ruff check skills/easybib/scripts/tex_scan.py`
Expected: 0.

- [ ] **Step 7: Commit**

```bash
git add skills/easybib/scripts/tex_scan.py tests/test_tex_scan.py tests/fixtures/tex/empty_cites/ tests/fixtures/tex/sections/
...
**Files:**
- Create: `skills/easybib/scripts/fuzzy_match.py`
...
- [ ] **Step 3: Implement `skills/easybib/scripts/fuzzy_match.py`**
...
Run: `ruff check skills/easybib/scripts/fuzzy_match.py`
Expected: 0.

- [ ] **Step 6: Commit**

```bash
git add skills/easybib/scripts/fuzzy_match.py tests/test_fuzzy_match.py
...
**Files:**
- Create: `skills/easybib/scripts/dblp_fetch.py`
...
- [ ] **Step 4: Implement `skills/easybib/scripts/dblp_fetch.py`**
...
Run: `ruff check skills/easybib/scripts/dblp_fetch.py`
Expected: 0.

- [ ] **Step 7: Commit**

```bash
git add skills/easybib/scripts/dblp_fetch.py tests/test_dblp_fetch.py tests/fixtures/dblp/
...
**Files:**
- Create: `skills/easybib/scripts/scholar_fetch.py`
...
- [ ] **Step 4: Implement `skills/easybib/scripts/scholar_fetch.py`**
...
Run: `ruff check skills/easybib/scripts/scholar_fetch.py`
Expected: 0.

- [ ] **Step 7: Commit**

```bash
git add skills/easybib/scripts/scholar_fetch.py tests/test_scholar_fetch.py tests/fixtures/scholar/
...
**Files:**
- Create: `skills/easybib/scripts/organize_bib.py`
...
- [ ] **Step 3: Implement `skills/easybib/scripts/organize_bib.py`**
...
Run: `ruff check skills/easybib/scripts/organize_bib.py`
Expected: 0.

- [ ] **Step 6: Commit**

```bash
git add skills/easybib/scripts/organize_bib.py tests/test_organize_bib.py
...
**Files:**
- Create: `skills/easybib/scripts/changelog.py`
- Create: `skills/easybib/scripts/config.py`
...
- [ ] **Step 4: Implement `skills/easybib/scripts/changelog.py`**
...
- [ ] **Step 5: Implement `skills/easybib/scripts/config.py`**
...
Run: `ruff check skills/easybib/scripts/changelog.py skills/easybib/scripts/config.py`
Expected: 0.

- [ ] **Step 8: Commit**

```bash
git add skills/easybib/scripts/changelog.py skills/easybib/scripts/config.py tests/test_changelog.py tests/test_config.py
...
**Files:**
- Create: `skills/easybib/SKILL.md`
- Create: `skills/easybib/DBLP.md`
- Create: `skills/easybib/MATCHING.md`
- Create: `skills/easybib/CONFIG.md`

- [ ] **Step 1: Create `skills/easybib/SKILL.md`**
...
- [ ] **Step 2: Create `skills/easybib/DBLP.md`**
...
`dblp_fetch.py` wraps both and caches responses in `.easybib-cache/dblp/` for 7 days.
...
- [ ] **Step 3: Create `skills/easybib/MATCHING.md`**
...
- [ ] **Step 4: Create `skills/easybib/CONFIG.md`**
...
```bash
git add skills/easybib/SKILL.md skills/easybib/DBLP.md skills/easybib/MATCHING.md skills/easybib/CONFIG.md
...
```markdown
Invoke the `easybib` skill in **check** mode.
...
```markdown
Invoke the `easybib` skill in **create** mode.
...
```markdown
Invoke the `easybib` skill in **organize** mode.
...
TEX_SCAN = Path("skills/easybib/scripts/tex_scan.py").resolve()
...
- `.easybib-cache/` — 7-day TTL cache of DBLP and Scholar responses. Add-listed in `.gitignore`.
